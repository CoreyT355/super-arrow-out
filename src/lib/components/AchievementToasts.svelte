...
{#if items.length > 0}
    ...
    >
        {#each items as toast (toast.uid)}
            ...
            >
                <span class="text-2xl leading-none shrink-0" aria-hidden="true">{toast.achievement.icon}</span>
                <div class="min-w-0">
                    <p class="text-[0.65rem] font-semibold uppercase tracking-widest {darkMode ? 'text-amber-400' : 'text-amber-500'}">
                        Achievement unlocked
                    </p>
                    <p class="text-sm font-bold truncate {darkMode ? 'text-white' : 'text-slate-900'}">{toast.achievement.title}</p>
                </div>
            </div>
        {/each}
    </div>
