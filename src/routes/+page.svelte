...
{#if gameState === 'menu'}
	<!-- ─── Start screen ─────────────────────────────────────────────────────── -->
	<main class="relative w-full h-dvh {darkMode ? 'bg-slate-900' : 'bg-slate-100'} flex flex-col items-center justify-center gap-6 p-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]">

		<!-- Gear button -->
		<button
			onclick={() => (menuSettingsOpen = true)}
			class="absolute right-4 flex items-center justify-center w-9 h-9 rounded-lg transition-colors
			       {darkMode
